...
std::vector<uint32_t> kmeanspp(MXd x, Func Dsq, uint32_t K, MXd c0, uint32_t K0, std::mt19937& rng, std::vector<double>& maxMinDists){
	assert (K>= K0);
	...
	if (K0 == 0){
		...
		for (uint32_t j = 0; j < x.rows(); j++){
			...
			maxMinDist = maxMinDist < mindists[j] ? mindists[j] : maxMinDist;
		}
		maxMinDists.push_back(maxMinDist);
	} else {
